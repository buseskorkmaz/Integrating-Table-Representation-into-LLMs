...
def main(task, model, peft, finetuned, log_file):
    base_model = f"google/flan-t5-xl"
    if finetuned:
        if peft:
            # Load the model and tokenizer
            tokenizer = AutoTokenizer.from_pretrained(base_model, trust_remote_code=True)
            load_model = AutoModelForSeq2SeqLM.from_pretrained(base_model, torch_dtype=torch.float16)
    ...
    test_dataset_path = os.path.join(root_path, f'data/t5_compliant_hf_{task}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--finetuned', type=bool, help='True for to load finetuned model False to use pre-trained model', default=True)
