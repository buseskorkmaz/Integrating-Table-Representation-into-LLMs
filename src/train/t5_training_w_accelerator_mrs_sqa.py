...
def main(model_name):
    ...
    train_cfg = {
        "save_checkpoint_dir": os.path.join(file_path, f'../../models/{model_name}-msr_sqa-1e-6'),
        "use_wandb" : "true",
    ...
    dataset_path = os.path.join(file_path, '../../data/t5_compliant_hf_msr_sqa')
    table_dataset = load_from_disk(dataset_path)
    dataset_eval = table_dataset['validation'].select_columns(["input_ids", "attention_mask", "labels"])
    dataset_train = table_dataset['train'].select_columns(["input_ids", "attention_mask", "labels"])
    ...
    # print(t5_cfg)
    # print("Tie word embed", t5_cfg.tie_word_embeddings)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', type=str, help='Model name: Huggingface id of flant5 family')
